...
exports.handleQuery = (searchtext) => {
    const heroCard1 = exports.getCustomAdaptiveCard(adaptivecard_1.adaptiveCardBody);
    const heroCard2 = exports.getCustomAdaptiveCard2(adaptivecard_1.adaptiveCardBody);
    // Writing 'all' in the search bar will display all cards stored
    if (!searchtext || searchtext.toLowerCase() === 'all') {
        return (exports.createPreviewList(data, heroCard1, heroCard2));
    }
    // Writing anything else in the search bar will filter the displayed cards
        ...
        return (exports.createPreviewList(queriedItems, heroCard1, heroCard2));
    }
};
// Function to process a list of items into a list of cards for output
exports.createPreviewList = (items, heroCard, heroCard2) => {
    let out = items.map((item, index) => {
        if (index % 2) {
            ...
            return (Object.assign({}, heroCard2, { preview: botbuilder_1.CardFactory.thumbnailCard(item.title, item.subTitle, [item.heroImageSrc]) }));
        }
    });
    return out;
};

...
exports.createPreviewList = (items) => {
    let out = items.map((item, index) => {
        ...
        return (Object.assign({}, heroCard, { preview: botbuilder_1.CardFactory.thumbnailCard(`${item.firstName} ${item.lastName}`, item.diagnosis, [`https://robohash.org/${item.firstName}.png?set=set5`]) }));
    });
    return out;
};

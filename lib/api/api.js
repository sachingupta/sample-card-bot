...
exports.createPreviewList = (items) => {
    let out = items.map((item, index) => {
        const heroCard = adaptivecard_1.getCustomAdaptiveCardPatient(item);
        return (Object.assign({}, heroCard, { preview: botbuilder_1.CardFactory.thumbnailCard(`${item.firstName} ${item.lastName}`, item.diagnosis, [`https://robohash.org/${item.firstName.toLowerCase()}.png?set=set5`]) }));
    });
    return out;
};

...
exports.createPreviewList = (items) => {
    let out = items.map((item, index) => {
        const heroCard = adaptivecard_1.createAdaptiveCard(item);
        return (Object.assign({}, heroCard, { contentType: 'AdaptiveCard', preview: botbuilder_1.CardFactory.thumbnailCard(`${item.firstName} ${item.lastName}`, item.diagnosis, [`https://robohash.org/${item.firstName}.png?set=set5`]) }));
    });
    return out;
};
